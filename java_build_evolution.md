...
如果你想进一步深入某个环节（比如 Maven 依赖解析、shade 资源冲突、Spring Boot classloader），我也可以继续写一篇更深入的文章。


---

## 6. Spring Boot 是如何使用插件打包并实现 fat jar 的

Spring Boot 的可执行 fat jar 并不是 Maven 原生能力，而是通过  
`spring-boot-maven-plugin` 实现的。
...
### 6.5 为什么 Spring Boot 选择“嵌套 jar”方案
...
### 6.6 Spring Boot fat jar vs Maven Shade
